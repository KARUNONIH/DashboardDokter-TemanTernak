...
const Input = ({ label, type, className, placeholder, onchange, error, value, checked }) => {
  ...
  return (
    <div className="mb-4 flex flex-col">
      ...
      <input type={type} id={slugLabel(label)} placeholder={placeholder} className={`${className} rounded border-2 border-gray-200 p-1.5 text-sm`} onChange={onchange} value={value} minLength={type === "password" ? 8 : 0}  required/>
